...
class Frame
{
  public:
   ...
   virtual Magick::Image  process(const uint_t)  throw (underflow_error) = 0;
...
class ImgFrame : public Frame
{
  public:
    ...
    Magick::Image  process(const uint_t)  throw (underflow_error);
...
Magick::Image  ImgFrame::process(const uint_t  trgt_)  throw (underflow_error)
{
    if (_imgs.empty()) {
